...
for currTrial=1:numTrials 
   ...
   %Reaction Time
    ...
    elseif GetSecs-stuffOnScreen>2
       %dummy variables
       currRT=888
       pressedKeyNum=888
       pressedKeyName=888
    ...
    D.RT(currTrial)=currRT
     ...
     if (currWordCategory==1) % word referred to animal
        if (pressedKeyNum==KbName('p')) % "animal" pressed
            respError=0;
     ...
     elseif (currWordCategory==2)
        if (pressedKeyNum==KbName('q')) % "object" pressed
            respError=0;
       ...
       Screen('FillRect', window.onScreen, [1000 1000 1000]);      
       Screen('Flip', window.onScreen); 
       pause(.5) %% this doesn't need to be that long I don't think;
      ...
      elseif respError==888
       ...
       D.expName{currTrial}='Experiment4Metamer_ResponseGroup2.m';
    ...
    %Wait (empty screen) before next trial--JITTERED
     timeoffsete = -0.5:0.25:0.5;
     waite=1+randsample(timeoffsete,1)
     Screen('FillRect', window.onScreen, [1000 1000 1000]);      
     Screen('Flip', window.onScreen); 
     pause(waite); 
